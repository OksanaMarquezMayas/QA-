...
function createEmail() {
    ...
    cy.wait(5000); // Consider using cy.wait until the element is visible instead of a fixed wait time
...
function confirmEmail() {
    ...
    // Get the first row in the table and find the first anchor tag
...
/*describe('User Registration Test with Ukrainian Names', () => {
  ...
  const randomPhone = `+380${getRandomInt(100000000, 999999999)}`;
  ...
  it('Registers a new user with only letters in names, confirms email and checks password confirmation', () => {
      ...
      cy.origin('https://teamchallenge-sport-store-frontend.vercel.app', () => {
          // 2. Перейти на сторінку реєстрації
          cy.visit('https://teamchallenge-sport-store-frontend.vercel.app/auth/signup'); 

          // 3. Заповнити форму реєстрації
          cy.get('input[name="firstName"]').type(randomFirstName); // Ім'я (тільки букви)
          cy.get('input[name="lastName"]').type(randomLastName); // Прізвище (тільки букви)
          cy.get('input[name="patronymic"]').type(randomPatronymic); // По батькові (тільки букви)
          cy.get('input[name="phone"]').type(randomPhone); // Телефон (рандом)
          cy.get('input[name="email"]').type(email); // Вставити згенеровану пошту
          cy.get('input[name="password"]').type(randomPassword); // Ввести пароль
          cy.get('input[name="confirmPassword"]').type(randomPassword); // Ввести підтвердження паролю
          cy.get('button[type="submit"]').click(); // Натиснути "Зареєструватися"
      });
